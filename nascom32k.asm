;==============================================================================
;
; Ported to Multibus 80/24
;
; Scott Baker, https://www.smbaker.com/, November 2024
;
;==============================================================================
;
...
; BASIC WORK SPACE LOCATIONS
        ...
        DEFC    WRKSPC  =   0E000H      ; <<<< BASIC Work space        
...
; BASIC ERROR CODE VALUES
        ...
        DEFC    TOPRAM  = 0FE00H ; Leave 512 bytes for monitor at top of RAM

; BASIC CODE COMMENCES

        ORG     2000H           ; Second rom in 80/24 board
...
SYSTEM: JP      EXITIS          ; Exit to system
...
XMSIZE:  LD      HL,MEMMSG       ; Point to message
...
SIGNON: DEFB    "8085 BASIC Ver 4.7c",CR,LF
...
DINPOS: POP     AF              ; Restore character
        POP     BC              ; Restore buffer length
        JP      RST08           ; Send it via RST 08

OUTNCR: CALL    OUTC            ; Output character in A
        JP      PRNTCRLF        ; Output CRLF

CLOTST: CALL    RST10           ; Get input character
        AND     01111111B       ; Strip bit 7
...
COUNT:  PUSH    HL              ; Save code string address
        ...
        CALL    RST10           ; Get input character
        CP      CTRLC           ; Is it control "C"?
...
TSTBRK: CALL    RST18           ; Check input status
        OR      A               ; Check count Zero
        RET     Z               ; No key, go back
        CALL    RST10           ; Get the key into A
        CP      ESC             ; Escape key?
        JP      Z,BRK           ; Yes, break
        CP      CTRLC           ; <Ctrl-C>
        JP      Z,BRK           ; Yes, break
...
MONITR: JP      EXITIS          ; No "monitor", so just exit ISIS
...
DEFC    XPOLL  = 0x003E
        ...
        ; print char with high bit stripped
RST08:  PUSH    AF
        AND     07FH
        CALL    XTYPE
        POP     AF
        ...
        ; get char
RST10:  CALL    XGETCH
	CP	60H		; automatic uppercase code
	JP	M,CIN1
	CP	7BH
	JP	P,CIN1
	SUB	20H		; end uppercase code
CIN1:   RET

        ; return a=0 if no charts ready
RST18:  JMP     XPOLL
