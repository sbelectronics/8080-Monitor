...
;	MEMORY ALLOCATION
;
EM	EQU	0FE80H	; TOP OF MEMORY + 1 = LIMIT (reserve no bytes for monitor)
NSCR	EQU	0	; NUMBER OF 1024 BYTE SCREENS
...
LES2	LXI	H,1	; ELSE X < Y
	JMP	HPUSH	; (S1) <- TRUE
;
	DB	82H	; U< ( UNSIGNED < )        : U< 2DUP XOR 0< IF DROP 0< 0= ELSE - 0< ;
	DB	'U'
	DB	'<'+80H
	DW	LESS-4
ULESS	DW	DOCOL,TDUP
...
#if NODISK
; NOTE - we need BLOCK because WORD calls it, though it's
;        inside an IF statement, so it should never get called.
BLOCK	DW	DOCOL,SEMIS
#else
...
COUT	PUSH    PSW
	MOV	A,C		; Character to print is in C
        ANI     07FH
        CALL    XOUT1		; Print character in A
        POP     PSW
        RET
